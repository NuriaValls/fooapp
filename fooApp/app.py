...
app.config['MONGO_DBNAME'] = 'ads-nuria'
app.config['MONGO_URI'] = 'mongodb://prova:prova1234@ds251332.mlab.com:51332/ads-nuria'
...
def product_edit(product_id):
    ...
    if product is None:
        # Abort with Not Found.
        abort(404)
    form = ProductFormEdit(request.form)
    if request.method == 'POST' and form.validate():
        mongo.db.products.update_one({"_id": ObjectId(product_id)}, {"$set": {'name': form.data['name'], 'description': form.data['description'], 'price': form.data['price']}}, True)
        return redirect(url_for('products_list'))
    form.fill(product)
